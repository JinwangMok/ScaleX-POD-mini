...
> 완전히 초기화된 베어메탈에서 `scalex get clusters`까지

### Quick Start: 대화형 설치 (권장)

단일 명령어로 인터뷰 기반 설정 → 의존성 설치 → 빌드 → 클러스터 구축까지 자동 완료:

```bash
# 대화형 TUI 설치 (whiptail/dialog 기반)
bash <(curl -fsSL https://raw.githubusercontent.com/JinwangMok/ScaleX-POD-mini/main/install.sh)
```

또는 다운로드 후 실행 (checksum 검증):
```bash
...
설치 스크립트는 5단계로 진행됩니다:
1. **Phase 0**: 의존성 자동 감지 및 설치 (Rust, Ansible, OpenTofu, kubectl, Helm 등)
2. **Phase 1**: 베어메탈 노드 SSH 접근 설정 (→ `.baremetal-init.yaml`, `.env` 생성)
3. **Phase 2**: SDI 가상화 레이어 설정 (→ `sdi-specs.yaml` 생성)
4. **Phase 3**: 클러스터 & GitOps 설정 (→ `k8s-clusters.yaml`, `secrets.yaml` 생성)
5. **Phase 4**: 레포 클론, CLI 빌드, 자동 프로비저닝 (`scalex facts → sdi init → cluster init → bootstrap`)

중단 시 자동 상태 저장 — 재실행하면 마지막 완료 단계부터 이어서 진행합니다.

> **수동 설치**를 선호하면 아래 Step-by-Step 가이드를 따르세요.
...
```
scalex-cli/                # Rust CLI — 603 tests, 0 clippy warnings
  src/commands/            #   facts, sdi, cluster, get, status, kernel-tune, secrets, validate
