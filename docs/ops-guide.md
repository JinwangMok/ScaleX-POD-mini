...
|----------|---------|-------------|
| `cd.jinwang.dev` | `http://argocd-server.argocd.svc.tower.local:80` | ArgoCD UI |
| `auth.jinwang.dev` | `http://keycloak.keycloak.svc.tower.local:80` | Keycloak OIDC |
| `tower-api.jinwang.dev` | `https://192.168.88.99:6443` | Tower K8s API (외부 kubectl 접근 시 필수) |
| `sandbox-api.jinwang.dev` | `https://192.168.88.109:6443` | Sandbox K8s API (외부 kubectl 접근 시 필수) |
...
- Tower K8s API: `https://tower-api.jinwang.dev` (**OIDC 설정 완료 후**에만 kubectl 가능)
- Sandbox K8s API: `https://sandbox-api.jinwang.dev` (**OIDC 설정 완료 후**에만 kubectl 가능)
...
>
> 1. kubectl이 `tower-api.jinwang.dev` 또는 `sandbox-api.jinwang.dev`에 연결 → CF Edge가 TLS 종단
...
cp _generated/clusters/tower/kubeconfig.yaml ~/.kube/tower-tailscale.yaml
sed -i 's|server: https://192.168.88.99:6443|server: https://<TAILSCALE_BASTION_IP>:6443|' ~/.kube/tower-tailscale.yaml
export KUBECONFIG=~/.kube/tower-tailscale.yaml
kubectl get nodes
```
