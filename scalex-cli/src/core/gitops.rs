...
/// Application definition for a cluster-specific generator.
#[cfg(test)]
pub struct GeneratorApp {
    pub app_name: String,
    pub namespace: String,
    pub sync_wave: String,
}

/// Generate an ApplicationSet YAML for a new cluster.
/// Pure function: takes cluster name, server URL, project name, and app list.
/// This enables adding a 3rd, 4th, ... cluster without manual YAML editing.
#[cfg(test)]
pub fn generate_cluster_generator_yaml(
    cluster_name: &str,
    server_url: &str,
    project_name: &str,
    repo_url: &str,
    ...
    let elements: String = apps
        .iter()
        .map(|app| {
            format!(
                "          - appName: {}\n            namespace: {}\n            syncWave: \"{}\"",
                app.app_name, app.namespace, app.sync_wave
            )
        })
        .collect::<Vec<_>>()
        .join("\n");
    ...
    format!(
        r#"# {cluster_name}-specific apps (auto-generated by scalex)
  ...
  template:
    ...
    spec:
      ...
      source:
        repoURL: {repo_url}
        targetRevision: main
        path: "gitops/{cluster_name}/{{{{.appName}}}}"
      destination:
        server: "{server_url}"
        namespace: "{{{{.namespace}}}}"
      syncPolicy:
...
/// Generate a common-generator YAML for a new cluster (deploys apps from gitops/common/).
#[cfg(test)]
pub fn generate_common_generator_yaml(
    cluster_name: &str,
    server_url: &str,
    project_name: &str,
    repo_url: &str,
    common_apps: &[GeneratorApp],
) -> String {
    let elements: String = common_apps
        .iter()
        .map(|app| {
            format!(
                "          - appName: {}\n            namespace: {}\n            syncWave: \"{}\"",
    ...
    format!(
        r#"# Common apps for {cluster_name} (auto-generated by scalex)
apiVersion: argoproj.io/v1alpha1
kind: ApplicationSet
metadata:
  name: {cluster_name}-common-apps
  namespace: argocd
spec:
  goTemplate: true
  generators:
...
{elements}
  template:
    metadata:
      name: "{cluster_name}-common-{{{{.appName}}}}"
      annotations:
        argocd.argoproj.io/sync-wave: "{{{{.syncWave}}}}"
    spec:
      project: {project_name}
...
"#;
    ...
    // ── Unit: Cluster generator YAML generation ──

    #[test]
    fn test_generate_cluster_generator_yaml_valid() {
        let apps = vec![
            GeneratorApp {
                app_name: "cluster-config".to_string(),
                namespace: "kube-system".to_string(),
                sync_wave: "0".to_string(),
            },
            GeneratorApp {
                app_name: "cilium".to_string(),
                namespace: "kube-system".to_string(),
                sync_wave: "1".to_string(),
            },
        ];

        let yaml = generate_cluster_generator_yaml(
            "datax",
            "https://datax-api:6443",
            "datax-project",
            "https://github.com/JinwangMok/ScaleX-POD-mini.git",
            &apps,
        );

        // Must be valid YAML
        let parsed: serde_yaml::Value = serde_yaml::from_str(&yaml)
            .unwrap_or_else(|e| panic!("generator YAML not valid: {e}\n{yaml}"));
        assert!(parsed.is_mapping());
    ...
    #[test]
    fn test_generate_common_generator_yaml_valid() {
        let apps = vec![GeneratorApp {
            app_name: "cert-manager".to_string(),
            namespace: "cert-manager".to_string(),
            sync_wave: "1".to_string(),
        }];

        let yaml = generate_common_generator_yaml(
            "datax",
            "https://datax-api:6443",
            "datax-project",
            "https://github.com/JinwangMok/ScaleX-POD-mini.git",
        ...
        let parsed: serde_yaml::Value = serde_yaml::from_str(&yaml)
            .unwrap_or_else(|e| panic!("common generator YAML not valid: {e}\n{yaml}"));
        assert!(parsed.is_mapping());

        assert!(yaml.contains("name: datax-common-apps"));
        assert!(yaml.contains("path: \"gitops/common/"));
        assert!(yaml.contains("appName: cert-manager"));
    }

    #[test]
    fn test_generate_cluster_generator_matches_sandbox_structure() {
        // Generated YAML for a new cluster must have the same structure as sandbox-generator
        let sandbox = include_str!("../../../gitops/generators/sandbox/sandbox-generator.yaml");
        let sandbox_parsed: serde_yaml::Value = serde_yaml::from_str(sandbox).unwrap();
        ...
        let generated = generate_cluster_generator_yaml(
            "newcluster",
            "https://newcluster-api:6443",
            "newcluster-project",
            "https://github.com/JinwangMok/ScaleX-POD-mini.git",
            &apps,
        );
        ...
        // Both must be ApplicationSet kind
        assert_eq!(
            sandbox_parsed["kind"].as_str(),
            gen_parsed["kind"].as_str(),
            "Generated must be same kind as sandbox"
        );
        assert_eq!(
            sandbox_parsed["apiVersion"].as_str(),
            gen_parsed["apiVersion"].as_str(),
            "Generated must have same apiVersion"
        );
