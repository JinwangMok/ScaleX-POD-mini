...
| 항목 | 상태 | 비고 |
|------|------|------|
| Rust 구현 | PASS | 253 tests, 0 clippy warnings |
...
**상태: PARTIAL — `sdi clean` 로직 분기 테스트 부재**
...
| `scalex sdi clean --hard` | CODE-ONLY | 존재 | **FAIL** | confirm 플래그, dry-run, 출력 디렉토리 분기 테스트 없음 |
| `scalex sdi sync` | PASS (7 tests) | 존재 | N/A | — |
...
### Sprint 8a: 오프라인 테스트 강화 — IN PROGRESS

> 목표: 물리 인프라 없이 검증 가능한 모든 로직 분기와 cross-config 정합성을 TDD로 커버

| # | Task | Checklist | 상태 |
|---|------|-----------|------|
| 8a.1 | `sdi clean --hard` 로직 분기 테스트 (confirm 플래그, dry-run, 출력 디렉토리) | CL-8, CL-13 | TODO |
| 8a.2 | `clean → rebuild` 멱등성 E2E dry-run 테스트 | CL-13 | TODO |
| 8a.3 | SDI spec ↔ K8s clusters cross-config pool 참조 검증 테스트 | CL-1, CL-8 | TODO |
| 8a.4 | `scalex get config-files` 상세 검증 테스트 | CL-8 | TODO |
| 8a.5 | 노드 cleanup 스크립트 내용 검증 테스트 | CL-8, CL-13 | TODO |
...
## Test Summary (253 tests — Sprint 7b 기준)

| Module | Tests | Coverage |
|--------|-------|----------|
| core/validation | 41 | pool mapping, cluster IDs, CIDR, legacy detection, single-node, baremetal, idempotency, E2E pipeline, sync |
...
| commands/sdi | 8 | network resolve, host infra, pool state |
| core/sync | 7 | sync diff, VM conflict detection |
