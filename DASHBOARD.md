...
1. **417개 테스트는 전부 순수 함수(pure function) 단위 테스트다.** HCL 생성, inventory 파싱, YAML 검증 등 데이터 변환 로직만 검증하며, 실제 SSH 연결, `tofu apply`, Kubespray 실행, kubectl 접근을 전혀 테스트하지 않는다.

2. **"코드가 존재한다" ≠ "기능이 동작한다."** `scalex sdi init`은 HCL을 생성하고 `tofu apply`를 호출하는 코드가 있지만, 실제 물리 노드에서 libvirt VM이 정상 생성되는지는 **한 번도 검증되지 않았다.**

3. **CF Tunnel을 통한 외부 kubectl은 구조적으로 불가능하다 (Keycloak 미설정 상태에서).** CF Tunnel은 TLS를 종단(terminate)하므로 클라이언트 인증서 인증이 전달되지 않는다. OIDC 토큰 인증만 통과 가능하며, Keycloak Realm/Client가 설정되지 않은 현재 상태에서는 CF Tunnel 경유 kubectl은 **작동하지 않는다.**
...
| SDI host 참조 검증 | ✅ 테스트 통과 (unknown host 탐지 + sdi init 파이프라인 연결) |
...
| CLI 일관성 | ✅ Sprint 25a: sdi init/clean/sync 모두 `--config`/`--env-file` 플래그 통일 |
| 431 tests, 0 clippy warnings | ✅ 전부 통과 |
| cargo fmt | ✅ 통과 |
...
- 테스트: `test_readme_installation_guide_all_steps` — 11개 단계 존재 검증 ✅
- 테스트: `test_readme_references_real_scalex_commands` — 모든 CLI 참조가 유효 ✅
...
| `scalex sdi clean --hard` | ✅ | ✅ (plan 로직, 12 tests) | ✅ (tofu destroy + SSH cleanup) | ❌ |
...
| 구분 | 원인 | 영향 범위 |
|------|------|----------|
| **테스트 한계** | 417개 테스트가 모두 순수 함수 테스트 → 실행 경로(I/O) 미검증 | 모든 실행 커맨드 |
...
### Sprint 18: 코드 수준 갭 해소 ✅ DONE (388 → 396 tests)

- [x] 18a: CLI 실행 경로 pipeline ordering 테스트
- [x] 18b: CF Tunnel 인증 경로 문서 검증 테스트
- [x] 18c: `.baremetal-init.yaml` 3가지 접근 방식 + 2가지 인증 방식 테스트

### Sprint 19: 구조 검증 + 확장성 테스트 ✅ DONE (396 → 408 tests)

- [x] 19a: SOCKS5 proxy manifest 구조 검증
- [x] 19b: GitOps 디렉토리 구조 검증
- [x] 19c: 2-Layer 템플릿 관리 검증
- [x] 19d: 단일 노드 SDI 풀 + 단일 클러스터 K8s config 검증

### Sprint 21: 엣지 케이스 검증 강화 ✅ DONE (408 → 414 tests)

- [x] 21a: 중복 클러스터 이름 검증 함수 + cluster init 파이프라인 연결
- [x] 21b: SDI 호스트 참조 검증 함수 + sdi init 파이프라인 연결
- [x] 21c: README External Access 섹션 확장
- [x] 21d: CLAUDE.md + ARCHITECTURE.md 수정
- [x] 21e: README CLI 일관성 + Installation Guide 전체 단계 검증 테스트

### Sprint 25: CLI 일관성 + 문서 정합성 ✅ DONE (414 → 431 tests)

- [x] 25a: `sdi clean`에 `--config`/`--env-file` 플래그 추가 (init/sync과 통일)
- [x] 25a: hardcoded `PathBuf::from("credentials/...")` 제거 검증 테스트
- [x] 25a: 모든 SDI 서브커맨드의 `config: PathBuf` 플래그 존재 검증 테스트
- [x] 25b: `build_pool_state` 순수 함수 — mixed host assignment 테스트
- [x] 25c: README.md 테스트 카운트 388→417 업데이트
- [x] 25c: CLAUDE.md 테스트 카운트 414→417 업데이트
- [x] 25d: DASHBOARD.md 정직한 재작성

### Sprint 29: 네트워크 검증 + Sync 안전성 강화 ✅ DONE (417 → 431 tests)

- [x] 29a: `validate_cluster_network_overlap()` 순수 함수 추출 — Pod/Service CIDR + DNS domain 중복 검증
- [x] 29a: `cluster init` 파이프라인에 네트워크 중복 검증 연결
- [x] 29a: 8개 테스트 (unique pass, pod/service/dns collision, multiple collision, 3-cluster pairwise, pipeline wiring, example config)
- [x] 29b: `ConflictSeverity` enum (Critical/High/Medium) + `classify_conflict_severity()` 순수 함수
- [x] 29b: 3개 테스트 (management=CRITICAL, control-plane=HIGH, worker=MEDIUM)
- [x] 29c: `has_management_cluster_conflict()` 순수 함수 — 관리 클러스터 VM 삭제 차단
- [x] 29c: `sdi sync`에 severity 라벨 + management hard-block 연결
- [x] 29c: 3개 테스트 (management conflict true/false, empty conflicts)

---

## 실행 계획 (남은 Sprint)

### Sprint 26: 실환경 E2E 검증 — SDI + Kubespray (⬜ 인프라 필요)
...
| core/validation (network) | 8 | CIDR overlap, DNS domain, pairwise, example config, pipeline wiring |
| core/sync (severity) | 7 | conflict severity (CRITICAL/HIGH/MEDIUM), management cluster protection |
| **TOTAL** | **431** | **순수 함수 테스트만 — 실행 경로 미포함** |
