...
- **Last stable commit**: `80024b7` (Sprint 10a — 310 tests)
...
### Sprint 11b: dry-run E2E 통합 테스트 (오프라인)

> **목표**: 실제 I/O 없이 전체 파이프라인(facts→sdi init→cluster init→secrets→gitops)의 dry-run 경로가 정상 동작함을 검증

| # | Task | 상태 |
|---|------|------|
| 11b-1 | dry-run 모드 전체 파이프라인 통합 테스트 | TODO |
| 11b-2 | config→output 생성물 cross-validation 강화 | TODO |
| 11b-3 | 에러 경로(잘못된 config, 누락 facts 등) 테스트 | TODO |
